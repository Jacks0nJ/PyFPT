...
def skewness_N_sto_limit(V, V_p, V_pp, phi_int, phi_end):
    ...
    skewness_value = \
        skewness_value/delta_N_squared_sto_limit(V, V_p, V_pp, phi_int,
                                                 phi_end)**1.5
...
def kurtosis_N_sto_limit(V, V_p, V_pp, phi_int, phi_end, Fisher=True):
    ...
    fourth_moment = fourth_central_moment_N_sto_limit(V, V_p, V_pp,
                                                      phi_int, phi_end)
    std = delta_N_squared_sto_limit(V, V_p, V_pp, phi_int, phi_end)
    if Fisher is True:
        return fourth_moment/std**2-3
    else:
        return fourth_moment/std**2
...
def pdf_gaussian_skew_kurtosis(x, mean, sigma, third_cumulant,
                               fourth_cumulant):
    norm_x = (x-mean)/sigma
