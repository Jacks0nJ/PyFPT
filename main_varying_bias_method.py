...
power_range = np.array([0.6,0.8,1,1.1,1.2,1.3])


bias = 1.

num_full_sims = len(power_range)
...
if storage == True:
    height_storage = np.zeros((num_bins,len(power_range)))
    bin_centres_storage = np.zeros((num_bins,len(power_range)))
    if log_normal == True:
        errors_storage = np.zeros((len(power_range), 2, num_bins))
    elif include_errors == 'yes':
        errors_storage = np.zeros((num_bins,len(power_range)))
    if contour == True:
        xedges_centre_storage = np.zeros((num_bins,len(power_range)))
        yedges_centre_storage = np.zeros((num_bins,len(power_range)))
        h_storage = np.zeros((len(power_range), num_bins, num_bins))
        
        
for j, power in enumerate(power_range):

    print('Now simulating for power = ' + str(power))
    
    
    num_sims = int(100000/mp.cpu_count())
    def V(phi):
        V = 0.5*(m*phi)**2
        return V
    ...
    def bias_function(phi):
        constant = bias/(2*PI*6**0.5)
        B_0_p = constant*(m/M_PL)*phi_i**(power-1)
        return B_0_p*phi**power
    ...
    if using_IS == 'yes':
        def multi_processing_func(phi_i, phi_r, phi_end, N_i, N_f, dN, bias_function,\
            ...
            Ns, ws =\
                is_code.many_simulations_importance_sampling(phi_i, phi_r, phi_end,\
                         N_i, N_f, dN, bias_function, num_sims, V, V_dif, V_ddif,\
                             bias_type = 'custom')
            '''
            Ns, ws =\
        ...
        if __name__ == "__main__":
            ...
            processes = [Process(target=multi_processing_func,\
                                 args=(phi_i, phi_r,  phi_end, 0.0, N_f, dN, bias_function,\
...
power_range_og = power_range
b_params = b_params[has_data_points_logic]
power_range_cut = power_range[has_data_points_logic]
...
if storage == True:
    ...
    for j in range(len(power_range_og)):
    ...
    my_file_name = 'combined_data_power_range_'\
                +('%s' % float('%.3g' % np.min(power_range)))+'_to_'\
                    +('%s' % float('%.3g' % np.max(power_range)))+'_with_m_'\
    ...
    if edgeworth_series == True:
        ax1.plot(bin_centres_combined, sci_stat.norm.pdf(bin_centres_combined,\
                ...
                label='{0}'.format('Gaussian'), color = CB_color_cycle[0])
    ...
    overlap_name = 'overlap_plot_m_'+str(m)+\
        '_power_range_log_'+('%s' % float('%.3g' % np.min(power_range_og)))+\
        '_to_'+('%s' % float('%.3g' % np.max(power_range_og))) +'_phi_UV_'+\
    ...
    if publication_plots == True:
        ...
        plt.title(str(len(power_range_og)) +r' $\times$ '+str(num_sims_used) +\
    ...
    if log_normal == False:
        ...
        plt.yscale('log')
        plt.legend()
        plt.title(str(len(power_range_og)) +r' $\times$ '+str(num_sims_used) + r', $dN$=' +\
              ('%s' % float('%.2g' % dN))+', m='+('%s' % float('%.3g' % m)))
        plt.legend(loc='lower left')
        plt.savefig(comp_language+'_results/'+'combined_plot_m_'+str(m)+\
            '_power_range_log_log_'+('%s' % float('%.3g' % np.min(power_range_og)))+\
            '_to_'+('%s' % float('%.3g' % np.max(power_range_og)))+'_v2.pdf',\
            transparent=True)
        plt.show()
        plt.close()
