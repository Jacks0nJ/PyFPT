'''
Importance Sampling Simulation 1-Dimension
------------------------------------------
This is the main module of the PyFPT code, as it runs the simulations, post
processes and exports the data ready for plotting, for a 1D FPT problem.
'''
...
from .importance_sampling_cython import\
    importance_sampling_simulations_1dim


def is_simulation_1dim(drift, diffusion, x_in, x_end,
                       ...
                       display=True):
    """Executes the simulation runs till ``x_end`` is crossed ``num_runs``
    times, then returns the histogram bin centres, heights and errors.
    ...
    def multi_processing_func(x_in, x_r, x_end, t_in, t_f,
                              time_step, bias, num_runs, queue_fpts,
                              queue_ws, queue_refs):
        results =\
            importance_sampling_simulations_1dim(x_in, x_r, x_end, t_in,
                                                 t_f, time_step, bias,
    ...
    processes = [Process(target=multi_processing_func,
                         args=(x_in, x_r,  x_end, t_in, t_f,
                               ...
                               queue_fpts, queue_ws, queue_refs))
    ...
    fpt_array = np.array([queue_fpts.get() for p in processes])
    ...
    if save_data is True:
        if bias_type == 'diffusion':
            file_label = '_diffusion_bias_' + ('%s' % float('%.3g' % bias))
